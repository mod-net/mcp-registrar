...
async fn test_registrar_register_server() {
    ...
    let list_result = registrar.handle("ListServers", serde_json::json!({})).await.unwrap();
...
async fn test_registrar_get_server() {
    ...
    let register_result = registrar.handle("RegisterServer", serde_json::to_value(request).unwrap()).await.unwrap();
...
async fn test_registrar_update_server_status() {
    ...
    let update_result = registrar.handle("UpdateServerStatus", update_params).await.unwrap();
...
async fn test_registrar_unregister_server() {
    ...
    let unregister_result = registrar.handle("UnregisterServer", unregister_params).await.unwrap();
...
async fn test_registrar_heartbeat() {
    ...
    registrar.handle("UpdateServerStatus", update_params).await.unwrap();
    
    // Send a heartbeat to make it active again
    let heartbeat_params = serde_json::json!({ "id": server_id });
    let heartbeat_result = registrar.handle("Heartbeat", heartbeat_params).await.unwrap();
