...
use schnorrkel::{Signature, PublicKey as Sr25519PublicKey};
use std::fs;
use std::path::Path;
...
impl HttpChainIndex {
    pub fn from_env() -> Result<Self, Error> {
        let base = std::env::var("CHAIN_INDEX_URL").map_err(|_| Error::InvalidState("CHAIN_INDEX_URL not set".into()))?;
...
impl ChainIndex for HttpChainIndex {
    async fn fetch_module_pointer(&self, module_id: &str) -> Result<ModulePointer, Error> {
        ...
        let resp = self.client.get(&url).send().await.map_err(|e| Error::Serialization(e.to_string()))?;
        if !resp.status().is_success() {
            return Err(Error::InvalidState(format!("chain index {} -> {}", url, resp.status())));
        }
        let mp: ModulePointer = resp.json().await.map_err(|e| Error::Serialization(e.to_string()))?;
        Ok(mp)
    }
}
...
impl FileChainIndex {
    pub fn from_env() -> Result<Self, Error> {
        let path = std::env::var("CHAIN_INDEX_FILE").map_err(|_| Error::InvalidState("CHAIN_INDEX_FILE not set".into()))?;
        Ok(Self { path })
    }
    ...
    fn load_map(&self) -> Result<HashMap<String, ModulePointer>, Error> {
        ...
        if let Some(map) = v.as_object() {
            ...
            for (k, mv) in map {
                if mv.is_object() {
                    ...
                    obj_map.entry("module_id".to_string()).or_insert(serde_json::Value::String(k.clone()));
                    if let Ok(mp) = serde_json::from_value::<ModulePointer>(serde_json::Value::Object(obj_map)) {
                        out.insert(k.clone(), mp);
                        any = true;
                    }
                }
            }
            if any { return Ok(out); }
        }
        // Try array [ mp, ... ]
        ...
        Err(Error::InvalidState("unsupported CHAIN_INDEX_FILE format".into()))
    }
}
...
pub fn verify_digest(bytes: &[u8], digest_str: &str) -> Result<(), Error> {
    ...
    // Try hex first
    let expected: Vec<u8> = if algo_trimmed.chars().all(|c| c.is_ascii_hexdigit()) && algo_trimmed.len() % 2 == 0 {
        (0..algo_trimmed.len())
            .step_by(2)
            .map(|i| u8::from_str_radix(&algo_trimmed[i..i+2], 16).unwrap_or(0))
            .collect()
    } else {
        general_purpose::STANDARD.decode(algo_trimmed).map_err(|e| Error::Serialization(e.to_string()))?
    };
...
pub fn decode_pubkey_from_owner(owner: &str) -> Result<[u8;32], Error> {
    ...
    if o.len() == 64 && o.chars().all(|c| c.is_ascii_hexdigit()) {
        let mut out = [0u8;32];
        for i in 0..32 { out[i] = u8::from_str_radix(&o[i*2..i*2+2], 16).map_err(|e| Error::Serialization(e.to_string()))?; }
        return Ok(out);
    }
    // Proper SS58 decode (AccountId 32 bytes). Format (common case): [address_type(1)] [pubkey(32)] [checksum(2)]
    let data = bs58::decode(o).into_vec().map_err(|e| Error::Serialization(e.to_string()))?;
    if data.len() != 35 { return Err(Error::InvalidState("unsupported SS58 length".into())); }
    ...
    let out = h.finalize();
    let cs = &out[0..2];
    if cs != checksum { return Err(Error::InvalidState("invalid SS58 checksum".into())); }
    let mut pk = [0u8;32];
...
pub fn verify_signature_sr25519(module_bytes: &[u8], digest_opt: &Option<String>, owner: &str, sig_b64_or_hex: &str) -> Result<(), Error> {
    ...
    let digest_bytes = if let Some(d) = digest_opt { 
        ...
        if body.chars().all(|c| c.is_ascii_hexdigit()) && body.len()%2==0 {
            (0..body.len()).step_by(2).map(|i| u8::from_str_radix(&body[i..i+2],16).unwrap_or(0)).collect::<Vec<u8>>()
        } else {
            general_purpose::STANDARD.decode(body).map_err(|e| Error::Serialization(e.to_string()))?
        }
    } else {
        let mut h = Sha256::new(); h.update(module_bytes); h.finalize().to_vec()
    };
    ...
    let sig_bytes = if sig_b64_or_hex.trim().chars().all(|c| c.is_ascii_hexdigit()) && sig_b64_or_hex.len() == 128 {
        (0..sig_b64_or_hex.len()).step_by(2).map(|i| u8::from_str_radix(&sig_b64_or_hex[i..i+2],16).unwrap_or(0)).collect::<Vec<u8>>()
    } else {
        general_purpose::STANDARD.decode(sig_b64_or_hex.trim()).map_err(|e| Error::Serialization(e.to_string()))?
    };
