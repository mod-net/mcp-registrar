...
fn render_key_json(kj: &KeyJson) -> serde_json::Value {
    ...
    if let Some(ba_hex) = kj.byte_array.as_ref() {
        ...
        let s = if ba_hex.starts_with("0x") || ba_hex.starts_with("0X") {
            &ba_hex[2..]
        } else {
            ba_hex.as_str()
        };
        if s.len() % 2 == 0 {
            let bytes: Vec<serde_json::Value> = (0..s.len())
                .step_by(2)
...
enum Commands {
    ...
    #[command(name = "key-save")]
    KeySave(KeySaveArgs),
    #[command(name = "key-load")]
    KeyLoad(KeyLoadArgs),
    #[command(name = "save")]
    Save(KeySaveArgs),
    #[command(name = "load")]
...
struct GenAllArgs {
    ...
    #[arg(long)]
    aura_name: Option<String>,
    #[arg(long)]
    grandpa_name: Option<String>,
}
#[derive(Args, Debug)]
struct MultisigArgs {
    #[arg(long)]
...
struct GetArgs {
    ...
    #[arg(long)]
    field: Option<String>,
    #[arg(value_enum, default_value_t=Scheme::Sr25519)]
    scheme: Scheme,
    #[arg(long, default_value = "substrate")]
    network: String,
}
...
fn require_subkey() {
    if which::which("subkey").is_err() {
        eprintln!("Error: 'subkey' not found on PATH");
        std::process::exit(1);
    }
}
...
fn parse_subkey(
    ...
    for line in output.lines().map(|l| l.trim()) {
        ...
        if l.starts_with("secret phrase") {
            phrase = line.split(':').nth(1).map(|s| s.trim().to_string());
        } else if l.starts_with("secret seed") {
            seed = line.split(':').nth(1).map(|s| s.trim().to_string());
        } else if l.starts_with("public key (hex)") {
            pubhex = line.split(':').nth(1).map(|s| s.trim().to_string());
        } else if l.starts_with("ss58 address") {
            ss58 = line.split(':').nth(1).map(|s| s.trim().to_string());
        }
    }
...
fn cmd_gen(a: GenArgs) -> anyhow::Result<()> {
    ...
    let kj = KeyJson {
        scheme: a.scheme.as_str().into(),
        network: a.network,
        byte_array: ss58
            .as_ref()
            .and_then(|s| ss58_to_bytes(s).ok())
            .map(|b| format!("0x{}", hex::encode(b))),
        mnemonic_phrase: None,
        secret_phrase: phrase,
        public_key_hex: pubhex,
        private_key_hex: seed,
        ss58_address: ss58.clone(),
        key_type: Some(a.scheme.as_str().into()),
        is_pair: Some(true),
        is_multisig: None,
        threshold: None,
        signers: None,
...
fn cmd_gen_all(a: GenAllArgs) -> anyhow::Result<()> {
    ...
    let base_dir = a
        .out_dir
        .clone()
        .unwrap_or_else(|| keys_dir().to_string_lossy().to_string());
...
fn cmd_get(a: GetArgs) -> anyhow::Result<()> {
    ...
    if let Some(ss58) = a.ss58_address.as_ref() {
        ...
        let kj = KeyJson {
            scheme: a.scheme.as_str().into(),
            network: a.network,
            byte_array: Some(format!("0x{}", hex::encode(pk_bytes))),
            mnemonic_phrase: None,
            secret_phrase: None,
            public_key_hex: Some(pubhex),
            private_key_hex: None,
            ss58_address: Some(ss58.clone()),
            key_type: Some("ss58".into()),
            is_pair: Some(false),
            is_multisig: None,
            threshold: None,
    ...
    fn wrapper_public_view(
        ...
        if let Some(ba_hex) = blob.byte_array.as_ref() {
            ...
            if s.len() % 2 == 0 {
                let arr: Vec<serde_json::Value> = (0..s.len())
                    .step_by(2)
                    .filter_map(|i| u8::from_str_radix(&s[i..i + 2], 16).ok())
                    .map(|b| serde_json::Value::from(b))
                    .collect();
    ...
    if let Some(nm) = a.name.as_ref() {
        let file = keys_dir().join(if nm.ends_with(".json") {
            nm.clone()
        } else {
            format!("{}.json", nm)
        });
    ...
    if let Some(inp) = a.input.as_ref() {
        if let Ok(pk_bytes) = ss58_to_bytes(inp) {
            ...
            let file = keys_dir().join(if inp.ends_with(".json") {
                inp.clone()
            } else {
                format!("{}.json", inp)
            });
    ...
    anyhow::bail!(
        "Provide --public-key 0x<hex>, --ss58-address <addr>, or a name (positional/--name)"
    )
}
...
fn cmd_multisig(a: MultisigArgs) -> anyhow::Result<()> {
    ...
    for s in &a.signer {
        let pk = ss58_to_bytes(s)?;
        signers_hex.push(pk.to_vec());
    }
    signers_hex.sort();
    let mut hasher = Blake2b512::new();
    hasher.update(b"modlpy/utilisig");
    for pk in &signers_hex {
        hasher.update(pk);
    }
    hasher.update(a.threshold.to_le_bytes());
...
fn cmd_key_save(a: KeySaveArgs) -> anyhow::Result<()> {
    let kj = if let Some(ph) = a.phrase.as_ref() {
        from_phrase(ph, a.scheme.as_str(), &a.network)?
    } else if let Some(pu) = a.public.as_ref() {
        from_public(pu, a.scheme.as_str(), &a.network)?
    } else {
        eprint!("Enter secret phrase: ");
        io::stderr().flush().ok();
        let p = read_line_hidden()?;
        if p.trim().is_empty() {
            anyhow::bail!("Secret phrase cannot be empty");
        }
        from_phrase(&p, a.scheme.as_str(), &a.network)?
    };
...
fn cmd_key_load(a: KeyLoadArgs) -> anyhow::Result<()> {
    ...
    let file_path = if let Some(f) = a.file.as_ref() {
        ...
        let fname = if nm.ends_with(".json") {
            nm.clone()
        } else {
            format!("{}.json", nm)
        };
        ...
        let i = s.trim().parse::<usize>().unwrap_or(0);
        if i >= rows.len() {
...
fn cmd_list() -> anyhow::Result<()> {
    ...
    let items: Vec<_> = rows
        ...
        .enumerate()
        .map(
            |(i, p)| serde_json::json!({"index":i,"file":p.file_name().unwrap().to_string_lossy()}),
        )
        .collect();
    let out = serde_json::json!({"keys_dir":keys_dir(),"items":items});
    println!("");
    println!("{}", serde_json::to_string_pretty(&out)?);
    Ok(())
}
...
fn from_phrase(phrase: &str, scheme: &str, network: &str) -> anyhow::Result<KeyJson> {
    ...
    Ok(KeyJson {
        ...
        byte_array: ss58
            ...
            .map(|b| format!("0x{}", hex::encode(b))),
        mnemonic_phrase: None,
        secret_phrase: Some(phrase.into()),
        public_key_hex: pubh,
        private_key_hex: seed,
        ss58_address: ss58,
        key_type: Some(scheme.into()),
        is_pair: Some(true),
        is_multisig: None,
        threshold: None,
        signers: None,
...
fn encrypt_key(kj: &KeyJson) -> anyhow::Result<EncBlobV1> {
    ...
    eprint!("Set password for key file: ");
    io::stderr().flush().ok();
    let pw1 = read_line_hidden()?;
    eprint!("Confirm password: ");
    io::stderr().flush().ok();
    let pw2 = read_line_hidden()?;
    if pw1 != pw2 {
        anyhow::bail!("Passwords do not match")
    }
    let mut key = [0u8; 32];
    scrypt::scrypt(pw1.as_bytes(), &salt, &params, &mut key)?;
    let nonce = {
        let mut n = [0u8; 12];
...
fn decrypt_key(blob: &EncBlobV1, password_opt: Option<&str>) -> anyhow::Result<KeyJson> {
    ...
    let pw = match password_opt {
        Some(p) => p.to_string(),
        None => {
            eprint!("Password for key file: ");
            io::stderr().flush().ok();
            read_line_hidden()?
        }
    };
    ...
    if let Some(s) = kj.byte_array.as_ref() {
        if s.starts_with("0x") == false {
            kj.byte_array = Some(format!("0x{}", s));
        }
    }
    Ok(kj)
}
...
fn ss58_to_bytes(addr: &str) -> anyhow::Result<[u8; 32]> {
    ...
    if data.len() != 35 {
        anyhow::bail!("unsupported SS58 length")
    }
    ...
    let out = h.finalize();
    if &out[..2] != checksum {
        anyhow::bail!("invalid SS58 checksum")
    }
    let mut pk = [0u8; 32];
