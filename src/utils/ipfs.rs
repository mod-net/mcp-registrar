...
fn gateway_base() -> String {
    env::ipfs_gateway_url().unwrap_or_else(|| "http://127.0.0.1:8080/ipfs/".to_string())
}
...
fn ipfs_base_url() -> String {
    env::ipfs_api_url().unwrap_or_else(|| "http://127.0.0.1:8000/api".to_string())
}
...
pub async fn fetch_ipfs_bytes(uri: &str) -> Result<Vec<u8>, Error> {
    let tail =
        strip_ipfs_scheme(uri).ok_or_else(|| Error::InvalidState("invalid ipfs uri".into()))?;
