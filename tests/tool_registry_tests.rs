...
async fn test_register_tool() {
    ...
    let reg = registry.handle("RegisterServer", serde_json::json!({"server_id": "test-server-2"})).await.unwrap();
...
async fn test_list_tools() {
    let registry = ToolRegistryServer::new();
    let reg = registry.handle("RegisterServer", serde_json::json!({"server_id": "test-server-3"})).await.unwrap();
    let server_id = reg["server_id"].as_str().unwrap().to_string();
    ...
    registry.handle("RegisterTool", serde_json::to_value(request).unwrap()).await.unwrap();
    ...
    let list_result = registry.handle("ListTools", serde_json::to_value(list_request).unwrap()).await.unwrap();
    ...
    let filter_result = registry.handle("ListTools", serde_json::to_value(filter_request).unwrap()).await.unwrap();
    ...
    let nonexistent_result = registry.handle("ListTools", serde_json::to_value(nonexistent_request).unwrap()).await.unwrap();
...
async fn test_get_tool() {
    let registry = ToolRegistryServer::new();
    let reg = registry.handle("RegisterServer", serde_json::json!({"server_id": "test-server-4"})).await.unwrap();
    let server_id = reg["server_id"].as_str().unwrap().to_string();
    ...
    let register_result = registry.handle("RegisterTool", serde_json::to_value(request).unwrap()).await.unwrap();
    ...
    let get_result = registry.handle("GetTool", serde_json::to_value(get_request).unwrap()).await.unwrap();
    
    // Verify the result
    assert_eq!(get_result["tool"]["id"].as_str().unwrap(), tool_id);
    assert_eq!(get_result["tool"]["name"].as_str().unwrap(), "GetTool");
    assert_eq!(get_result["tool"]["description"].as_str().unwrap(), "A test tool for getting");
}
...
async fn test_invoke_tool() {
    ...
    // Find echo tool
    let list = registry.handle("ListTools", serde_json::json!({})).await.unwrap();
    let tools = list["tools"].as_array().unwrap();
    let echo = tools.iter().find(|t| t["id"].as_str() == Some("echo") || t["name"].as_str() == Some("Echo")).unwrap();
    ...
    let resp = registry.handle("InvokeTool", serde_json::to_value(req).unwrap()).await.unwrap();
