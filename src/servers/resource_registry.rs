use crate::models::resource::{Resource, ResourceQuery, ResourceQueryResult, ResourceType};
...
impl ResourceRegistryServer {
    ...
    async fn query_resource(&self, query: ResourceQuery) -> Result<ResourceQueryResult, String> {
        ...
        if server_endpoint.starts_with("http://") || server_endpoint.starts_with("https://") {
            ...
            match self.http.post(&server_endpoint).json(&body).send().await {
                Ok(resp) => {
        ...
        Err(format!(
            "Unsupported endpoint for server {}: {}",
            resource.server_id, server_endpoint
        ))
    }
        ...
        match name {
            "RegisterResource" => {
                ...
                match self.register_resource(request) {
                    Ok(resource_id) => Ok(serde_json::to_value(RegisterResourceResponse {
                        resource_id,
                    })?),
