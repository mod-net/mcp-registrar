...
async fn invoke_echo_process_tool_via_registry() {
    ...
    assert!(tools.len() >= 1, "expected at least one tool loaded from manifests");
    let echo = tools
        .iter()
    ...
    let content = resp["result"]["result"]["content"].as_array().expect("content array");
...
async fn invoke_echo_with_missing_param_fails_validation() {
    ...
    // Find echo tool id
    let list_resp = registry.handle("ListTools", serde_json::json!({})).await.unwrap();
    ...
    let invocation = ToolInvocation { tool_id: echo_id, parameters: serde_json::json!({}), context: None };
    let req = InvokeToolRequest { invocation };
    let err = registry
        .handle("InvokeTool", serde_json::to_value(req).unwrap())
        .await
