...
pub fn keys_dir() -> PathBuf {
    if let Ok(dir) = std::env::var("MODSDK_KEYS_DIR") {
        return PathBuf::from(dir);
    }
    if let Ok(dir) = std::env::var("MODNET_KEYS_DIR") {
        return PathBuf::from(dir);
    }
    if let Ok(home) = std::env::var("HOME") {
        return PathBuf::from(home).join(".modnet/keys");
    }
    dirs::home_dir()
...
pub fn module_api_url() -> Option<String> {
    if let Ok(v) = std::env::var("MODSDK_MODULE_API_URL") {
        return Some(v);
    }
...
pub fn ipfs_api_url() -> Option<String> {
    if let Ok(v) = std::env::var("MODSDK_IPFS_API_URL") {
        return Some(v);
    }
    if let Ok(v) = std::env::var("IPFS_API_URL") {
        return Some(v);
    }
    // Backward compat
...
pub fn ipfs_api_key() -> Option<String> {
    std::env::var("IPFS_API_KEY").ok()
}

pub fn ipfs_gateway_url() -> Option<String> {
    if let Ok(v) = std::env::var("MODSDK_IPFS_GATEWAY_URL") {
        return Some(v);
    }
    if let Ok(v) = std::env::var("IPFS_GATEWAY_URL") {
        return Some(v);
    }
...
pub fn registry_cache_dir() -> PathBuf {
    if let Ok(p) = std::env::var("REGISTRY_CACHE_DIR") {
